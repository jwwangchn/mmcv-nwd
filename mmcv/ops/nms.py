...
class WassersteinNMSop(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, bboxes, scores, iou_threshold, offset, constant):
        inds = ext_module.wasserstein_nms(
            bboxes, 
            scores, 
            iou_threshold=float(iou_threshold), 
            offset=offset,
...
class GIoUNMSop(torch.autograd.Function):

    @staticmethod
    def forward(ctx, bboxes, scores, iou_threshold, offset):
        inds = ext_module.giou_nms(
            bboxes, 
            scores, 
            iou_threshold=float(iou_threshold), 
            offset=offset)
...
class DIoUNMSop(torch.autograd.Function):

    @staticmethod
    def forward(ctx, bboxes, scores, iou_threshold, offset):
        inds = ext_module.diou_nms(
            bboxes, 
            scores, 
            iou_threshold=float(iou_threshold), 
            offset=offset)
...
class CIoUNMSop(torch.autograd.Function):

    @staticmethod
    def forward(ctx, bboxes, scores, iou_threshold, offset):
        inds = ext_module.ciou_nms(
            bboxes, 
            scores, 
            iou_threshold=float(iou_threshold), 
            offset=offset)
...
@deprecated_api_warning({'iou_thr': 'iou_threshold'})
def wasserstein_nms(boxes, scores, iou_threshold, offset=0, constant=12.8):
    """Dispatch to either CPU or GPU NMS implementations.
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = ext_module.wasserstein_nms(*indata_list, **indata_dict)
    else:
        inds = WassersteinNMSop.apply(boxes, scores, iou_threshold, offset, constant)
    dets = torch.cat((boxes[inds], scores[inds].reshape(-1, 1)), dim=1)
    if is_numpy:
        dets = dets.cpu().numpy()
        inds = inds.cpu().numpy()
...
@deprecated_api_warning({'iou_thr': 'iou_threshold'})
def giou_nms(boxes, scores, iou_threshold, offset=0):
    assert isinstance(boxes, (torch.Tensor, np.ndarray))
    assert isinstance(scores, (torch.Tensor, np.ndarray))
    is_numpy = False
    if isinstance(boxes, np.ndarray):
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = ext_module.giou_nms(*indata_list, **indata_dict)
    else:
        inds = GIoUNMSop.apply(boxes, scores, iou_threshold, offset)
    dets = torch.cat((boxes[inds], scores[inds].reshape(-1, 1)), dim=1)
    if is_numpy:
        dets = dets.cpu().numpy()
        inds = inds.cpu().numpy()
...
@deprecated_api_warning({'iou_thr': 'iou_threshold'})
def ciou_nms(boxes, scores, iou_threshold, offset=0):
    """Dispatch to either CPU or GPU NMS implementations.
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = ext_module.ciou_nms(*indata_list, **indata_dict)
    else:
        inds = CIoUNMSop.apply(boxes, scores, iou_threshold, offset)
    dets = torch.cat((boxes[inds], scores[inds].reshape(-1, 1)), dim=1)
    if is_numpy:
        dets = dets.cpu().numpy()
        inds = inds.cpu().numpy()
...
@deprecated_api_warning({'iou_thr': 'iou_threshold'})
def diou_nms(boxes, scores, iou_threshold, offset=0):
    """Dispatch to either CPU or GPU NMS implementations.
    ...
    if torch.__version__ == 'parrots':
        ...
        inds = ext_module.diou_nms(*indata_list, **indata_dict)
    else:
        inds = DIoUNMSop.apply(boxes, scores, iou_threshold, offset)
    dets = torch.cat((boxes[inds], scores[inds].reshape(-1, 1)), dim=1)
    if is_numpy:
        dets = dets.cpu().numpy()
        inds = inds.cpu().numpy()
